...
<div class="container">
	...
	<form id="dataFrm" method="post" action="/myapp/data/editOk" enctype="multipart/form-data">
		...
		<ul>
			...
			<li>
				<!-- 첫번째 첨부파일 -->
				<div>${vo.filename1}&nbsp;&nbsp;&nbsp;<b>X</b>
				</div> <!-- X를 누르면 삭제파일명이 있는 input의 name속성값을 del파일로 준다. --> <input
				type="hidden" name="" value="${vo.filename1}"> 
				<!-- X를 누르면 파일이 삭제되고 새로운 파일 선택이 가능하도록 input 만들어준다. -->
				<input type="hidden" name="filename" id="filename1">
			</li>
			<li>
				<!-- 두번째 첨부파일 있을때 --> <c:if
					test="${vo.filename2!=null && vo.filename2!=''}">
					<div>${vo.filename2}&nbsp;&nbsp;&nbsp;<b>X</b>
					</div>
